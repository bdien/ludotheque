from conftest import AUTH_ADMIN, AUTH_ADMIN_ID, fake_auth_user
...
def test_book():
    with db:
        ...
        user = User.get(id=AUTH_ADMIN_ID)

    # Book
    response = client.post("/bookings", json={"item": item.id}, headers=AUTH_ADMIN)
    ...
    # Unbook
    response = client.delete(f"/bookings/{booking_id}", headers=AUTH_ADMIN)
    assert response.status_code == 200
...
def test_book_BOOKING_MAXITEMS():
    ...
    for i in range(BOOKING_MAXITEMS):
        response = client.post(
            "/bookings", json={"item": items[i].id}, headers=AUTH_ADMIN
        )
        assert response.status_code == 200

    # Next one should be denied
    response = client.post("/bookings", json={"item": items[-1].id}, headers=AUTH_ADMIN)
    assert response.status_code == 403


def test_book_nonexistant_item():
    response = client.post("/bookings", json={"item": 666}, headers=AUTH_ADMIN)
    assert response.status_code == 400


def test_unbook_without_book():
    response = client.delete("/bookings/1234", headers=AUTH_ADMIN)
    assert response.status_code == 400
...
def test_unbook_another_user():
    ...
    response = client.delete(f"/bookings/{booking.id}", headers=AUTH_ADMIN)
    assert response.status_code == 400
